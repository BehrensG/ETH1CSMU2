...
scpi_result_t SCPI_MeasureQ(scpi_t * context)
{
	if((DC == bsp.config.mode) || (AC == bsp.config.mode))
	{
		if(SELECT_ADC1 == bsp.config.measure.adc_type)
		{
			...
			SCPI_ResultCharacters(context, TCP_Package(bsp.adc[ADC_VOLTAGE].meas, 0, bsp.config.measure.count), TCP_PACKGE_SIZE);
			SCPI_ResultCharacters(context, TCP_Package(bsp.adc[ADC_CURRENT].meas, 0, bsp.config.measure.count), TCP_PACKGE_SIZE);
		}
